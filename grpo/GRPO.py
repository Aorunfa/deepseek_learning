...
def prepare_dataset(split="train"):
    ...
    data = load_dataset('/local/dev1/chaofeng/gsm8k', 'main')[split]
    formatted_data = []
...
def extract_answer_from_dataset(text):
    ...
    if "####" not in text:
        return None
    return text.split("####")[1].strip() # 提取####后的数值答案
...
def evaluate_model(model, tokenizer, eval_examples, device):
    ...
    for example in eval_examples:
        ...
        outputs = model.generate(
            ...
            forced_eos_token_id=tokenizer.eos_token_id,             # 强制停止符号
            early_stopping=True                                     # 第一次触发停止
        )
        response = tokenizer.decode(outputs[0], skip_special_tokens=True)
        
        # Extract the predicted answer from the model output.
        try:
            predicted = extract_answer_from_model_output(response)   ## <answer> predicted </answer>
            
            # Check correctness in multiple ways
            if predicted == expected:  # First try exact match       ### 完全精确匹配
                is_correct = True
            else:
                # Try single number
...
def correctness_reward(prompts, completions, answer, **kwargs):     # 答案准确性奖励
    """
...
def format_reward(completions, **kwargs):                           # 推理格式patten存在性奖励
    """
    ...
    for response in responses:                                       # 只判断是否存在pattern
        score = 0.0
...
def combined_reward(prompts, completions, answer):                  # 两类奖励合并
    """
...
def compute_log_probabilities(model, input_ids, attention_mask, logits_to_keep):
    ...
    """
    # Run the model forward pass and obtain logits. 只保留最后logits_to_keep+1个logts输出
    logits = model(
        input_ids=input_ids,
    ...
    return selective_log_softmax(logits, input_ids) # 选择对应token_id的sofmax_logits
...
def generate_completions(model, tokenizer, prompts, num_generations=4, max_completion_length=32):
    ...
    # Repeat each prompt num_generations times. 对每个prompt需要采用num_generations个生成轨迹
...
def main():
    ...
    model_name = "/local/dev1/chaofeng/Qwen2.5-0.5B-Instruct" # The 0.5B model is not smart enough
