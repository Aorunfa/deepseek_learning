...
*MLA优化原理：*   
存储kv变成存储一个更低维的中间态`kv_laten = h * Wkv_a`, `Wkv_a`对h进行降维, h为行向量。后续再由kv_laten乘以对应升维矩阵得到k和v。此时kv缓存存储k-v键值对，变成了存储一个kv在隐空间的更低维度的向量，降低的显存消耗。

*MLA位置编码解耦：*   
ROPE旋转位置编码按照维度进行解耦，划分无位置编码与有位置编码两个部分，最后计算softmax前对没有位置编码和有位置编码的查询结果进行相加。

有点类似residual短连接效果，同时位置标记只由部分维度执行，减少全局影响的同时提高计算效率。

注意这里单独对位置编码后的k进行了kv缓存。我理解是对压缩后的laten升维后在做ROPE效果应该没有这样好。
...
另一方面，当下一token出现偏离时，容易会导致回答跑飞，考虑下下token，下下下token的可能性有利于当前token的预测纠偏。因此这种带有一定跨度的预见直观上可以进一步提高监督的强度
...
- 串行的MTP结构最开始用于推理的解码过程，用来提高推理的速度，deepseekV3在训练时采用，更符合推理模式。
...
首先定义了一个网格，用于启动并行执行线程，每个线程获得对应网格点对应的id（一维网格只有序数，二维有行列序数）, 以执行fp8反量化矩阵乘法为例进行说明
```python
...
# 二. r1
*背景:*    
上一个版本的r1-zero已经具备了长思维链的推理能力，但是容易出现多语言、无法出现结束符、答案描述难以理解等问题，r1在这些问题上做了优化。同时，r1的实验结果表明，对于小模型思维链推理能力，直接蒸馏来着大模型的推理过程数据比使用强化学习更好。

[r1](https://github.com/deepseek-ai/DeepSeek-R1)使用v3作为基础模型，使用GROP强化学习方法强化模型思维链推理能力，奖励函数使用规则函数，使得在标签数据较少，计算资源较少的的情况下也能学习到知识并泛化。
...
## 强化学习本质思路
可以理解为初始价值状态出发，每一个时间步，‘人’通过制定一个策略与环境进行交互获得奖励，策略具有一定的正影响或负影响，导致下一个时间步的价值发生变化。奖励可以是正的或负的
...
## 强化学习在llm后训练的本质
训练好的llm在进行推理时，首先是得到一组logits probs，然后在这组logits probs里面进行采样得到一个token id作为next token的输出。

强化学习目的是调整llm每一次的logits probs分布，使得其在采样以及在轨迹生成时能够得到符合预设偏好的结果。

这里强化学习的调整方法也很直接，通过采样完整的生成轨迹，计算总体的奖励，计算每一个轨迹点的交互优势，以此衡量每一次logits probs分布里面对应采样点的优势，以此对完成交互过程的监督以及logits分布的调整
...
同时考虑训练稳定性，每次新模型的输出分布与旧模型(或参考输出)尽可能相似，使用kloss进行度量。[论文地址](https://arxiv.org/abs/1707.06347)，[解析](https://mp.weixin.qq.com/s/y7o9F9vz8dv609ee6xqYtw)。

这里采样一组query-response对应为模型和环境进行多次交互，形成一条完整的交互轨迹，每个自回归的token生成是模型与环境的一次交互。

这里一次做了多次交互后，根据结果计算奖励，采样轨迹内执行价值函数。与“强化学习的本质思路”中每交互一次评价一次有所区别，这种先广泛交互后进行每次交互效果评估的方式称为广义优势估计GAE。

价值函数在这里是评估每一个轨迹点的优势变化，用于评估轨迹的好坏。查看源码可以发现，这里的的价值函数为原模型去掉输出头，增加了一个线性层。

### PPO在LLM后训练的要素介绍
> 策略模型 : 待训练的目标模型

> 参考模型 : 基于规则或基于模型，输出参考策略，用于对照目标模型的输出策略，避免每次每次策略模型的偏离过大，保证稳定收敛

> 奖励模型 : 对生成的query-response进行打分, response来自于策略模型推理生成。奖励模型应该是可提供训练偏好指导的

> 价值模型 : 基于模型，评估在已生成response t个token下，生成的第t+1个token的价值 (即st状态的价值)。常用设置为去输出头参考模型 + FFN，直观上看是第t+1位置的输出，考虑了前面t个的信息。但这样的设置我理解为是人为预设的

> T : 轨迹采样的最大长度，对应最大的生成序列长度

> 策略分布 : 自回归每次生成下一个token时，logits probs的分布对应为当前轨迹点的策略分布

> 状态st : 当前输入的prompt + 历史生成了的第t-1个token，作为第t次交互的状态
...
trl库实现流程: 
- 生成响应: *策略模型*根据prompt自回归生成完整response，长度不超过T，按batch统一补全到T。和query拼接得到query-response，同时得到对应的logitprobs。获得一条采样轨迹(rollout)

- 生成参考: *参考模型*根据query-response进行前向传播得到ref_logitprobs。这里使用**tecaher force**的方式

- 奖励打分: *奖励模型*对query-response进行打分，每一条采样轨迹对应一个奖励分数

- 价值计算: *价值模型*对response采样轨迹评估每一个采样点的价值, score[t]表示做出t时刻决策前的价值评估
...
PPO的价值函数本身就是价值评估的一种近似，对每个step token的价值估计本身质量并不高。相较之下，监督训练过的奖励模型打分奖励质量更高，直接使用奖励打分作为最终的生成优势估计在一定程度上能避免低质量的价值估计干扰

PPO的价值函数本身就是价值评估的一种近似，对每个step token的价值估计本身质量并不高。相较之下，监督训练过的奖励模型打分奖励质量更高，直接使用奖励打分作为优势估计在一定程度上能避免低质量的价值估计干扰

另一方面，grpo优化目标变成了结果导向的最大化奖励，而不需使用一个不好判断的价值模型考虑过程路径的价值变化。直观上更符合机器学习的本质。当数据量足够大时，应该至少是有一些效果的
另一方面，grpo优化目标变成了结果导向的最大化奖励，而不需使用一个不好判断的价值模型考虑过程路径的价值变化。直观上更符合机器学习的本质。当数据量足够大时，应该至少是有一些效果的
...
R1训练:  四个阶段，两个sft两个grpo，使用基于规则和准确度的奖励模型 
R1训练:  四个阶段，两个sft两个grpo，使用基于规则和准确度的奖励模型 
- SFT 少量cot数据冷启动
- GRPO 通过语言一致性奖励缓解混合语言问题. 奖励函数增加语言混合判断函数
- GRPO 通过语言一致性奖励缓解混合语言问题. 奖励函数增加语言混合判断函数
- SFT 过滤高质量cot数据和通用数据
