...
这里是一次做了多次交互后，根据结果计算奖励，采样轨迹内执行价值函数。与“强化学习的本质思路”中每交互一次评价一次有所区别，这种先广泛交互后进行每次交互效果评估的方式称为广义优势估计GAE。
...
**而使用组内采样轨迹的相对优势作为优势评估，‘相对’在一定方面比较了不同采样轨迹的优劣，通过这种比较一定程度上替代了价值模型对采用轨迹的监督**
